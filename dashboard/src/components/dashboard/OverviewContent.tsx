  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-1">
        <h2 className="text-xl font-bold text-white sm:text-2xl">Welcome back, Sefat!</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card hoverEffect>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-medium text-neutral-400">Total Blogs</CardTitle>
          ...
          <CardContent className="mt-2">
            <span className="text-3xl font-extrabold text-white">{totalBlogs}</span>
          </CardContent>
        </Card>
        ...
        <Card hoverEffect>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div>
              <CardTitle className="text-sm font-medium text-neutral-400">Unread Messages</CardTitle>
          ...
          <CardContent className="mt-2">
            <span className="text-3xl font-extrabold text-white">{unreadMessages}</span>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between border-b border-neutral-900 pb-4">
            <div>
              ...
              <p className="text-[11px] text-neutral-400">Latest showcase work uploaded</p>
            </div>
            <Link
              href="/projects"
          ...
          <div className="space-y-4">
            {recentProjects.length === 0 ? (
              <p className="text-xs text-neutral-500 py-6 text-center">No projects uploaded yet.</p>
            ) : (
              recentProjects.map((project) => (
                <div
                  key={project.id}
                  className="group flex gap-4 items-center rounded-xl bg-[#09090b] p-3 border border-neutral-900/60 hover:border-neutral-800 transition-colors"
                >
                  <img
                    src={project.image || "https://picsum.photos/seed/default/100/100"}
                    alt={project.title}
                    className="h-12 w-16 object-cover rounded-lg bg-neutral-900 border border-neutral-800"
                  />
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between border-b border-neutral-900 pb-4">
            <div>
              <h3 className="font-bold text-white text-base">Recent Articles</h3>
              <p className="text-[11px] text-neutral-400">Latest insights published</p>
            </div>
            <Link
              href="/blogs"
              className="inline-flex items-center gap-1 text-xs font-semibold text-[#E85D04] hover:text-[#C64F03] hover:underline"
          ...
          <div className="space-y-4">
            {recentBlogs.length === 0 ? (
              <p className="text-xs text-neutral-500 py-6 text-center">No blogs published yet.</p>
            ) : (
              recentBlogs.map((blog) => (
                <div
                  key={blog.id}
                  className="flex gap-4 items-center rounded-xl bg-[#09090b] p-3 border border-neutral-900/60 hover:border-neutral-800 transition-colors"
                >
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 items-center mt-1">
                      ...
                      <span className="text-[10px] text-neutral-400">{blog.date}</span>
                    </div>
                  </div>
                </div>
              ))
